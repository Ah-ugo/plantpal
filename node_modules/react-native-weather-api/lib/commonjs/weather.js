...
exports.fiveDaysForecast = fiveDaysForecast;
...
async function dailyForecast(args) {
  let result;
  let URL = 'https://api.openweathermap.org/data/2.5/onecall?lat=' + args.lat + '&lon=' + args.lon + '&exclude=hourly,minutely&appid=' + args.key + '&units=' + args.unit + '&lang=' + args.lang;
  await fetch(URL).then(res => res.json()).then(data => {
    result = data;
  });
  return result;
}
async function fiveDaysForecast(args) {
  let result;
  let URL = 'https://api.openweathermap.org/data/2.5/forecast?lat=' + args.lat + '&lon=' + args.lon + '&appid=' + args.key + '&units=' + args.unit + '&lang=' + args.lang;
  await fetch(URL).then(res => res.json()).then(data => {
    result = data;
  });
  return result;
